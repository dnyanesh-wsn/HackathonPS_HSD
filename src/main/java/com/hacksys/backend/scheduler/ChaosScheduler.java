...
public class ChaosScheduler {
    ...
    public void nightlyStockSyncWorker() {
        ...
        try {
            int delta = 999;
            int currentStock = inventoryService.getStock("PROD-005"); // WHY: fetch current stock before deducting to guard against negative result
            if (currentStock - delta < 0) { // WHY: reject delta that would drive stock below zero
                logStore.error(SVC, traceId, "NEGATIVE_STOCK",
                    "stock sync: delta rejected — would cause negative stock for PROD-005 currentStock=" + currentStock + " delta=" + delta);
                return;
            }
            inventoryService.deductStock("PROD-005", delta, traceId);
            logStore.info(SVC, traceId, "stock sync: delta applied successfully product=PROD-005 qty=" + delta);
        } catch (Exception e) {
