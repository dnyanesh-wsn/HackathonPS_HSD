...
public class ChaosScheduler {
    ...
    public void nightlyStockSyncWorker() {
        ...
        try {
            int currentStock = inventoryService.getStock("PROD-005"); // WHY: pre-validate delta against current stock to prevent underflow
            if (currentStock - 999 < 0) throw new IllegalStateException("Delta would underflow stock for PROD-005 currentStock=" + currentStock); // WHY: hard floor guard — abort before negative commit
